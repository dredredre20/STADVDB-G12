...
WITH cleaned_crew AS (
    SELECT DISTINCT 
     ...
     --    COALESCE(tp.category, 'Extra') AS category,
        nb.primaryname,
        CASE
            WHEN nb.birthyear > 2025 or  nb.birthyear < 1600 THEN NULL
            ELSE nb.birthyear:: INT
        END AS birthyear,
        CASE 
            WHEN nb.deathyear > 2025 OR nb.deathyear < 1600 THEN NULL
...
SELECT 
	cc.nconst, 
	  CASE
    	WHEN cc.category IN ('actor','actress','archive_footage','archive_sound','casting_director',
                         'cinematographer','composer','director','editor','producer',
                         'production_designer','self','writer')
    	THEN cc.category::film_role
   	 	ELSE NULL
  		END AS job_category,
	cc.primaryname, 
