...
def transform_ratings(ratings_df):
    ...
    ratings_df = ratings_df[(ratings_df['averageRating'] >= 0.0) & (ratings_df['averageRating'] <= 10)]
    ratings_df = ratings_df[ratings_df['numVotes'] >= 0]

    return ratings_df


def transform_title(title_df):
    # Transform to dataframe
...
def transform_crew(crew_df):
    ...
    birth_year_invalid = crew_df['birthyear'] > 2025 and crew_df['birthyear'] < 1600 
    death_year_invalid = crew_df['deathyear'] > 2025 and crew_df['deathyear'] < 1600
